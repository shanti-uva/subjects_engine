  ...
  task cleanup: :environment do
    ...
    if default_author_id.blank?
      ...
      Description.order('id').each do |d|
        ...
        if size<=140
          c = Caption.create language_id: d.language_id, content: d.content, author_id: author_id, feature_id: d.feature_id
          if !c.nil?
            description_ids << d.id
            d.destroy
          end
        elsif size<=750
          s = Summary.create language_id: d.language_id, content: d.content, author_id: author_id, feature_id: d.feature_id
          if !s.nil?
            description_ids << d.id
            d.destroy
          end
